    ...
    def password_set_click():
        def pass_dicide_click():
            ...
            if messagebox.OK:
                # 現在のパスワードを表示
                if password == "":
                    password_now_label.configure(
                        text='パスワードを設定していません', font=("", 9))
                else:
                    password_now_label.configure(
                        text='現在のパスワード:%s' % password, font=("", 9))
        ...
        if gformlock.flg == 0:
            ...
            # pass_win.grab_set()
            # ウインドウの×を押したときの処理
            pass_win.protocol("WM_DELETE_WINDOW",
                              lambda: formlock_off(pass_win))
            # パスワード設定ラベル
            ...
            # 決定ボタン
            pass_decide_btn = tk.Button(
                pass_win, text='決定', command=lambda: pass_dicide_click())
    ...
    def limit_set_click():
        ...
        def limit_dicide_click():
            ...
            if limit == '':
                ...
                if messagebox.OK:
                    if limit == "":
                        limit_now_label.configure(
                            text='制限時間を設定していません', font=("", 9))
                    else:
                        limit_now_label.configure(
                            text='現在の制限時間:%s分' % limit, font=("", 9))
        ...
        if gformlock.flg == 0:
            ...
            # limit_win.grab_set()
            # ウインドウの×を押したときの処理
            limit_win.protocol("WM_DELETE_WINDOW",
                               lambda: formlock_off(limit_win))
            limit_win.focus_set()
            # 制限時間設定ラベル
            ...
            warning_label.place(x=0, y=40)
            # 決定ボタン
            limit_decide_btn = tk.Button(
                limit_win, text='決定', command=lambda: limit_dicide_click())
...
# ----------------------------------------------------------------------------------
    ...
    # パスワード設定
    password_set_label = tk.Label(
        setting_frame, text='パスワードを設定できます', font=("", 12))
    password_set_btn = tk.Button(
        setting_frame, text='パスワード設定', command=lambda: password_set_click())
    # 現在のパスワード
    if f_password == "":
        password_now_label = tk.Label(
            setting_frame, text='パスワードを設定していません', font=("", 9))
    else:
        password_now_label = tk.Label(
            setting_frame, text='現在のパスワード:%s' % f_password, font=("", 9))
    # 制限時間の設定
    limit_set_label = tk.Label(
        setting_frame, text='制限時間を設定できます', font=("", 12))
    limit_set_btn = tk.Button(
        setting_frame, text='制限時間設定', command=lambda: limit_set_click())
    if f_limit == "":
        limit_now_label = tk.Label(
            setting_frame, text='制限時間を設定していません', font=("", 9))
    else:
        limit_now_label = tk.Label(
            setting_frame, text='現在の制限時間:%s分' % f_limit, font=("", 9))
    ...
    password_set_btn.place(x=btn_place, y=0, width=100,
                           height=30)  # form_xの2/5の位置に配置
    # 制限時間設定の配置
